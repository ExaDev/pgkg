-- Replace ts_rank_cd with proper BM25 scoring in pgkg_search().
--
-- ts_rank_cd uses cover density ranking which ignores inverse document
-- frequency (IDF) — a common term like "the" gets the same weight as a
-- rare term like "pgvector".  BM25 fixes this by weighting terms by how
-- informative they are (IDF) and normalising for document length.
--
-- The BM25 formula:
--   score(D, Q) = Σ IDF(qi) * tf(qi,D) * (k1+1) / (tf(qi,D) + k1*(1-b+b*|D|/avgdl))
--
-- Parameters: k1 = 1.2, b = 0.75 (standard defaults, hardcoded below).
--
-- Compatible with PostgreSQL 14+ (no unnest(tsquery) — uses text parsing
-- of plainto_tsquery output).
...
-- BM25 support: extract stemmed lexemes from the query.
-- plainto_tsquery('english', 'running fast')::text produces 'run' & 'fast'
-- We split on ' & ' and strip the surrounding single-quotes.
query_lexemes AS (
...
-- BM25 support: corpus-level statistics for the namespace.
-- n_total = active proposition count; avgdl = average document length.
-- length(tsv) returns distinct-lexeme count — a reasonable proxy for
-- document length on short texts (atomic propositions, typically 5-20 words).
corpus_stats AS (
    SELECT
...
-- BM25 support: document frequency per query lexeme (corpus-wide).
-- Uses 'simple' dictionary to avoid double-stemming already-stemmed lexemes.
doc_freq AS (
    SELECT
        ql.lexeme,
        (
...
-- BM25 support: IDF per query lexeme.
-- Robertson-Sparck Jones formula: ln((N - df + 0.5) / (df + 0.5) + 1)
idf AS (
    SELECT
        df.lexeme,
...
-- 1. Keyword retrieval with BM25 scoring (replaces ts_rank_cd)
-- GIN index on tsv handles the initial @@ filter; BM25 rescores.
